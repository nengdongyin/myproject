...
static int sensor_read(void *drv, uint16_t local_id, param_value_t *value)
{
    sensor_device_t *s = (sensor_device_t *)drv;

    switch (local_id) {
    case SENSOR_IP_EXPOSURE:
...
static int sensor_write(void *drv, uint16_t local_id, param_value_t value)
{
    sensor_device_t *s = (sensor_device_t *)drv;
...
static int sensor_exec(uint16_t local_id, param_value_t arg)
{
    (void)arg;
    switch (local_id) {
    case SENSOR_IP_SHUTDOWN:
