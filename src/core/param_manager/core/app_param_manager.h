...
#define PARAM_MODULE_REGISTER(_name, _init)                             \
    __attribute__((used, section(".rodata.param_modules")))              \
    static const param_module_reg_t _pm_auto_reg_##_name = {             \
        .init = (_init)                                                  \
    }
...
#define PARAM_UINT(_name, _id, _flags, _def, _min, _max)           \
    static param_range_entry_t _name = {                             \
        .base = { (_id), &app_vtable },                          \
        .type = PARAM_TYPE_UINT, .flags = (_flags), .dirty = 0,     \
        .has_range = ((_min) < (_max)),                             \
        .cache       = { .u32 = (_def) },                           \
...
#define PARAM_INT(_name, _id, _flags, _def, _min, _max)            \
    static param_range_entry_t _name = {                             \
        .base = { (_id), &app_vtable },                          \
        .type = PARAM_TYPE_INT, .flags = (_flags), .dirty = 0,      \
        .has_range = ((_min) < (_max)),                             \
        .cache       = { .i32 = (_def) },                           \
...
#define PARAM_FLOAT(_name, _id, _flags, _def, _min, _max)          \
    static param_range_entry_t _name = {                             \
        .base = { (_id), &app_vtable },                          \
        .type = PARAM_TYPE_FLOAT, .flags = (_flags), .dirty = 0,   \
        .has_range = ((_min) < (_max)),                             \
        .cache       = { .f32 = (_def) },                           \
...
#define PARAM_ENUM(_name, _id, _flags, _def, _vals, _cnt)          \
    static param_enum_entry_t _name = {                              \
        .base = { (_id), &app_vtable },                          \
        .type = PARAM_TYPE_ENUM, .flags = (_flags), .dirty = 0,     \
        .cache       = { .i32 = (_def) },                           \
        .default_val = { .i32 = (_def) },                           \
...
#define PARAM_BOOL(_name, _id, _flags, _def)                       \
    static param_bool_entry_t _name = {                              \
        .base = { (_id), &app_vtable },                          \
        .type = PARAM_TYPE_BOOL, .flags = (_flags), .dirty = 0,     \
        .cache       = { .b = (_def) },                             \
        .default_val = { .b = (_def) },                             \
        PARAM_DEBUG_NAME_INIT(_name)                                 \
    }
